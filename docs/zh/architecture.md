...
- 知识库：SQLite 元数据加 Milvus 向量检索
- 工具：可选 MCP 工具与内置知识 / 联网搜索工具
...
4. 文档和 chunk 元数据写入 SQLite。
5. 如果 Milvus 可用，则写入向量索引。
...
flowchart TD
    ...
    G --> H["生成 embedding"]
    H --> I["写入 Milvus"]
    I --> J["返回 doc_id + chunk_count + source_path"]
```
...
- `src/scriptwriter/knowledge/metadata_store.py`：SQLite 元数据与原文管理
